...
{ 
  ...
  for(sim in 1:n.sim)
  {
    ...
    {
      vaksineringsrekkefølge <- c('50-59år',
                                  '40-49år',
                                  '30-39år',
                                  '20-29år',
                                  '10-19år')
      ...
      # '70-79år',
      # '60-69år')  #'0-9år',
      vaksinegruppe.alder <- c(n50t59,n40t49,n30t39,n20t29,n10t19)
      vaksine.dekning.strategi <- c(vaksine.dekning.eldre50t59,vaksine.dekning.eldre40t49,
                                    vaksine.dekning.eldre30t39,vaksine.dekning.eldre18t29,vaksine.dekning.eldre18t29 )
    }
    ...
    if(vaksine.strategi >= 2)
    {
      vaksineringsrekkefølge <- c('20-29år',
                                  ...
                                  '40-49år',
                                  '50-59år')
      vaksinegruppe.alder <- c(n20t29,n10t19,n30t39,n40t49,n50t59)
      vaksine.dekning.strategi <- c(vaksine.dekning.eldre18t29,vaksine.dekning.eldre18t29*0.2,
                                    vaksine.dekning.eldre30t39 ,vaksine.dekning.eldre40t49,vaksine.dekning.eldre50t59)
    ...
    for(i in 1:n.dager)
    {
      ...
      de.som.smittes <- as.integer(rownames(de.forsøkes.smittes[(de.forsøkes.smittes$boks == "S") |(de.forsøkes.smittes$boks == "V") ,]))
      ...
      if(i >= tid.til.vaksine) 
        ...
        if(j < length(vaksineringsrekkefølge))
        {
          ...
          # else if(daglige.vaksiner > length(vaksinegruppe) & length(vaksinegruppe) > 0)  # Hvis man har mer vaksiner enn hva som er igjen i aldersgruppen
          # {
          #   index.som.vaksineres <- vaksinegruppe  # Vaksiner resten av aldersgruppen
          #   if(j < length(vaksineringsrekkefølge)) # Ikke vaksinert alle.
          #   {
          #     j = j + 1  #Gå til neste vaksinegruppe basert på strategi
          #     vaksinegruppe2 <- which(df.befolkning$aldersgruppe == vaksineringsrekkefølge[j] & (df.befolkning$boks == "S" | df.befolkning$boks == "RU"))  # Finn vaksinekandidater
          #     index.som.vaksineres <- c(index.som.vaksineres, sample(vaksinegruppe2, (daglige.vaksiner - length(vaksinegruppe)) ,replace = FALSE)) # legg til et et utvalg av nye gruppen
          #   }
          #   n.friske.vaksineres <- length(which(df.befolkning$boks[index.som.vaksineres] == "RU"))
          #   n.mottagelige.vaksineres <- length(which(df.befolkning$boks[index.som.vaksineres] == "S"))
          #   
    ...
    {
      somePDFPath = paste("F:/DATA/Analyse/Nils/R-beregninger/R-pdf/Resultater individbasert vaksinemodell/Output simuleringer/IVM m vaks eldre, n.dager",n.dager, "imp =",n.snitt.daglig.import,"R =",round(mean(R.Verdier), digits = 2),"k =",k,"DV =",daglige.vaksiner,"seed",seed.var,"vaks.strat",vaksine.strategi,"sim",sim,".pdf")
      pdf(file=somePDFPath, paper="USr",width=11, height=8.50)
      par(mfcol = c(1,1))
  
      write.xlsx(simulering, paste("F:/DATA/Analyse/Nils/R-beregninger/R-pdf/Resultater individbasert vaksinemodell/Output simuleringer/IVM m vaks eldre, n.dager" ,n.dager, "imp =",n.snitt.daglig.import,"R =",round(mean(R.Verdier), digits = 2),"k =",k,"DV =",daglige.vaksiner,"seed",seed.var,"vaks.strat",vaksine.strategi,"sim",sim,".xlsx"))
    }
    if(start.etter.vaks.eldre != TRUE)
    {
      somePDFPath = paste("F:/DATA/Analyse/Nils/R-beregninger/R-pdf/Resultater individbasert vaksinemodell/Output simuleringer/IVM, n.dager",n.dager, "imp =",n.snitt.daglig.import,"R =",round(mean(R.Verdier), digits = 2),"k =",k,"DV =",daglige.vaksiner,"seed",seed.var,"vaks.strat",vaksine.strategi,"sim",sim,".pdf")
      pdf(file=somePDFPath, paper="USr",width=11, height=8.50)
      par(mfcol = c(1,1))
  
      write.xlsx(simulering, paste("F:/DATA/Analyse/Nils/R-beregninger/R-pdf/Resultater individbasert vaksinemodell/Output simuleringer/IVM, n.dager",n.dager, "imp =",n.snitt.daglig.import,"R =",round(mean(R.Verdier), digits = 2),"k =",k,"DV =",daglige.vaksiner,"seed",seed.var,"vaks.strat",vaksine.strategi,"sim",sim,".xlsx"))
